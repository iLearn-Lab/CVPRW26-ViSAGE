...
def main():
    ...
    run_dir = os.path.join(parent_dir, 'checkpointstopdown', f'run_{run_timestamp}')
    ...
    for epoch in range(epochs):
        ...
        for videos, gts, fixations in train_bar:
            ...
            optimizer.zero_grad()
            preds, aux_preds = model(videos, return_aux=True)

            main_loss = criterion(preds.float(), gts, fixations=fixations)

            aux_loss = preds.new_tensor(0.0)
            for name, aux_pred in aux_preds.items():
                aux_loss = aux_loss + aux_loss_weights[name] * criterion.dense_only_loss(
                    aux_pred.float(), gts
                )
        ...
        if avg_cc > best_cc:
            ...
            for k, v in full_state.items():
                ...
                if k.startswith('decoder.') or k.startswith('dim_reducers.'):
