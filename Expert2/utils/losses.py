...
class VSPLoss(nn.Module):
    def __init__(self, kl_weight=10.0, cc_weight=2.0, sim_weight=1.0, nss_weight=0.0, bce_weight=1.0):
        super().__init__()
    ...
    def _dense_components(self, pred, target, fixations=None, bce_weight=None):
        if bce_weight is None:
            bce_weight = self.bce_weight

        B, _, T, _, _ = pred.shape
        pred = torch.clamp(pred, min=1e-7, max=1.0 - 1e-7)
        target = torch.clamp(target, min=0.0, max=1.0)

        p = pred.transpose(1, 2).reshape(B * T, -1)
        g = target.transpose(1, 2).reshape(B * T, -1)

        p_norm = p / (p.sum(dim=1, keepdim=True) + 1e-7)
        g_norm = g / (g.sum(dim=1, keepdim=True) + 1e-7)

        kl = torch.sum(g_norm * torch.log(g_norm / (p_norm + 1e-7) + 1e-7), dim=1).mean()

        p_centered = p - p.mean(dim=1, keepdim=True)
        g_centered = g - g.mean(dim=1, keepdim=True)
        cc = (p_centered * g_centered).sum(dim=1) / torch.sqrt(
            (p_centered ** 2).sum(dim=1) * (g_centered ** 2).sum(dim=1) + 1e-7
        )
        cc_loss = (1.0 - cc).mean()
        ...
        if bce_weight > 0:
            ...
            if fixations is not None:
                ...
                bce_loss = (bce_raw * weight_map).mean()
            else:
    ...
    def dense_only_loss(self, pred, target, bce_weight=0.0):
        return self._dense_components(pred, target, fixations=None, bce_weight=bce_weight)

    def forward(self, pred, target, fixations=None):
        dense_loss = self._dense_components(pred, target, fixations=fixations, bce_weight=self.bce_weight)

        loss_nss = pred.new_tensor(0.0)
        if fixations is not None and self.nss_weight > 0:
            B, _, T, _, _ = pred.shape
