...
def main():
    ...
    run_dir = os.path.join(parent_dir, 'checkpoints_stage2claude2', f'run_{run_timestamp}')
    ...
    stage1_path = 'stage1_best.pth' 
    
    if os.path.exists(stage1_path):
        
        stage1_weights = torch.load(stage1_path, map_location='cpu')

        stage1_weights.pop('decoder.center_bias.xx', None)
        stage1_weights.pop('decoder.center_bias.yy', None)
    ...
    for epoch in range(epochs):
        ...
        if avg_cc > best_cc:
            best_cc = avg_cc
            best_model_path = os.path.join(run_dir, "vsp_stage2_best.pth")
            ...
            torch.save(stage2_state_dict, best_model_path)
