# 文件路径: VSP/models/internvideo2_wrapper.py
import sys
import os
import torch
...
class InternVideo2Wrapper(nn.Module):
    ...
    def forward(self, x):
        ...
        features = []
        cls_token = None
        for idx, feat in enumerate(self.intermediate_features):
            if idx == len(self.intermediate_features) - 1:
                cls_token = feat[:, 0, :]  # (B, 3200)
